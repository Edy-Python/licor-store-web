...
def main(page: ft.Page):
    ...
    def agregar_producto(e):
        ...
        try:
            ...
            page.snack_bar = ft.SnackBar(ft.Text("Error al agregar el producto.", color=ft.Colors.WHITE), bgcolor=ft.Colors.RED)
            page.snack_bar.open = True
            page.update()
    ...
    def procesar_venta(e):
        ...
        try:
            ...
            # --- 1. VALIDAR STOCK Y BLOQUEAR LAS FILAS EN LA MISMA TRANSACCIÓN ---
            # "FOR UPDATE" bloquea las filas leídas hasta el commit/rollback,
            # así dos ventas simultáneas del mismo producto no pueden
            # descontar stock que ya no existe (evita sobreventa).
            items = []
            ...
            # (Calculamos el total sumando los subtotales de cada ítem en vez
            # de leerlo del texto ya formateado de lbl_total_pen/usd, que es
            # más frágil ante cualquier cambio de formato del label.)
    ...
    def procesar_excel(e: ft.FilePickerResultEvent):
        ...
        try:
            ...
            page.snack_bar = ft.SnackBar(ft.Text(f"❌ Error al procesar el Excel: {ex}", color=ft.Colors.WHITE), bgcolor=ft.Colors.RED)
            page.snack_bar.open = True
            page.update()
    ...
    def exportar_excel(e):
        try:
            ...
            # archivo se guarda en la carpeta "assets" del servidor y se abre
            # con launch_url para que el usuario lo descargue desde su
            # navegador; una ruta absoluta del servidor no le serviría.
            ...
            page.snack_bar = ft.SnackBar(
                content=ft.Text("¡Excel generado con éxito! Se abrió en una pestaña nueva.", color=ft.Colors.WHITE, weight=ft.FontWeight.BOLD),
            ...
            page.snack_bar = ft.SnackBar(ft.Text(f"❌ Error al exportar: {ex}", color=ft.Colors.WHITE), bgcolor=ft.Colors.RED)
            page.snack_bar.open = True
            page.update()
